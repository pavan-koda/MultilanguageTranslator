...
# ctranslate2 (Intel MKL/oneDNN) and torch each spin up their own OpenMP thread
# pool; on this many-core Xeon the two contend and corrupt the heap, crashing
# with a segfault during translation. Pin both to a single thread before either
# library initializes (must be set before they're imported).
...
# encode characters like '→' or Devanagari script used in log messages below.
if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='backslashreplace')
    sys.stderr.reconfigure(encoding='utf-8', errors='backslashreplace')

# Point argostranslate at its own subfolder so other model dirs don't interfere.
# Both package_data_dir (where installs are written) and package_dirs (where
# installed packages are scanned from) must point here, or installs land in
# argostranslate's OS-default dir while lookups keep checking this empty one.
_argos_dir = Path(__file__).parent / "models" / "argos_packages"
_argos_dir.mkdir(parents=True, exist_ok=True)
argostranslate.settings.package_data_dir = _argos_dir
...
_glossary_hi = {}    # hi->en: Devanagari term -> English phonetic sounding (from <tag>s only)
...
IT3_GGUF_REPO = "trysem/IndicTrans3-beta-Q4_K_M-GGUF"   # HF repo for pre-built GGUF (public, ungated)
...
def _load_it3_gguf():
    ...
    try:
        ...
        # Do NOT use torch.cuda — PyTorch 2.x dropped CC 5.x (Maxwell) support
        # so torch.cuda.is_available() returns False on K2200 even though
        # llama-cpp-python's ggml-cuda.dll works fine on CC 5.0+.
        n_gpu_layers = 0
        try:
            import llama_cpp as _lc
            _cuda_dll = Path(_lc.__file__).parent / 'lib' / 'ggml-cuda.dll'
            if _cuda_dll.exists():
                import subprocess
                smi = subprocess.run(
                ...
                if smi.returncode == 0:
                    ...
                    cc_str   = parts[1].strip() if len(parts) > 1 else '?'
                else:
                    vram_mb, cc_str = 4096, '?'
                # Reserve headroom for KV cache (n_ctx=2048) + inference buffers.
                # mb_per_layer ~100 MB (2.7 GB model / 28 layers).
                # CC 5.x (Maxwell): larger overhead reservation — older arch uses
                # more scratch memory and has no CUDA graphs optimisation.
                cc_parts = cc_str.split('.')
                cc_major  = int(cc_parts[0]) if cc_parts else 0
                overhead_mb  = 1500 if cc_major < 6 else 700
                free_mb      = max(0, vram_mb - overhead_mb)
                mb_per_layer = 100
                max_layers   = int(free_mb / mb_per_layer)
                # Cap at 90% of what fits so GPU doesn't peg at 100% —
                # remaining layers run on CPU, giving the GPU breathing room.
                n_gpu_layers = min(28, int(max_layers * 0.90))
                print(f"GGUF: CUDA enabled (CC {cc_str}), VRAM={vram_mb} MB "
                      f"— offloading {n_gpu_layers}/28 layers to GPU")
            else:
                print("GGUF: ggml-cuda.dll not found — running on CPU")
        except Exception as _e:
            print(f"GGUF: CUDA detection failed ({_e}) — running on CPU")
        ...
        _it3_gguf_model = Llama(
            ...
            n_ctx        = 2048,   # 4096 KV cache didn't fit alongside 2.7 GB weights
...
_GGUF_TEMPLATE_RE = re.compile(
    r'<\|[^|>]{1,40}\|>|<start_of_turn>\w*\n?|<end_of_turn>\n?'
)
...
def _strip_gguf_artifacts(text):
    ...
    # outputs a numbered plan or "Hindi Translation:" header before the content.
    for marker in ('**Hindi Translation:**', 'Hindi Translation:', '**Translation:**', 'Translation:'):
        if marker in text:
            text = text[text.index(marker) + len(marker):]
            break
    # Strip markdown bold (**text**) and bullet list markers (* text)
    import re as _re
...
def _translate_single_line_gguf(line, src_label, tgt_label, gloss_prefix):
    ...
    turns. The GGUF's embedded chat template is broken (emits <|assistant|>
    garbage), so the turn markers are written manually here. Few-shot
    completion prompts make the model echo the example instead of translating.
    """
    formatted = (
    ...
    # Model sometimes prefixes a preamble line ("यहाँ अनुवाद है:" / "Here is the
    # translation:") before the actual content — drop it, keep everything else.
    out_lines = [l for l in output.splitlines() if l.strip()]
    if out_lines and re.match(
        r'^\s*(यहाँ.{0,30}अनुवाद.{0,10}[:：]|Here (is|\'s) the translation.{0,10}[:：]?)\s*$',
        out_lines[0],
    ):
...
def _translate_with_it3_gguf(text, from_lang, to_lang):
    ...
    for line in lines:
        ...
        if result and to_lang == 'hi':
            import re as _re2
            m = _re2.match(r'^([^\w]*)([A-Za-z][A-Za-z0-9 &/\-()+]+):\s*(.+)', result, _re2.DOTALL)
            if m:
                lead = m.group(1)
                eng_label = m.group(2).strip()
                rest = m.group(3)
                hi_label = _glossary.get(eng_label)
                if not hi_label:
                    hi_label = next((v for k, v in _glossary.items()
...
def _rebuild_glossary():
    ...
    _glossary_hi = dict(_overrides.get('learnt_hi', {}))
    for term in _overrides.get('deleted_hi', []):
        _glossary_hi.pop(term, None)
...
def _normalize_terminal_suffix(suffix, result, to_lang):
    """Avoid gluing the source language's terminal punctuation onto a result that
    already ends with its own (the model usually generates proper target-language
    punctuation on its own) — e.g. hi "।" pasted after an EN sentence that already
    ends in ".", or an EN "." pasted after a hi sentence that already ends in "।"."""
    if not suffix:
        return suffix
    stripped = suffix.strip()
    term_chars = _TERMINATORS_HI | _TERMINATORS_EN
    if not stripped or any(c not in term_chars for c in stripped):
        return suffix  # empty/whitespace-only, or contains non-terminal symbols — leave as-is

    trailing_ws = suffix[len(suffix.rstrip()):]
    result_stripped = result.rstrip()
    target_terms = _TERMINATORS_HI if to_lang == 'hi' else _TERMINATORS_EN if to_lang == 'en' else None
    if target_terms is None:
        return suffix
    if result_stripped and result_stripped[-1] in target_terms:
        return trailing_ws  # result already ends properly — drop the source's terminator
    return ('।' if to_lang == 'hi' else '.') + trailing_ws


def _is_word_char(ch):
    """True for letters/digits AND combining marks (Devanagari matras, anusvara,
    virama, nukta, ...). Plain \\w excludes combining marks, which would otherwise
    get sheared off as "trailing symbols" — e.g. the "ें" in "करें।" — corrupting
    the word left behind and leaking the fragment untranslated into the output."""
    if ch == '_':
        return True
...
def _symbol_wrapper(text):
    ...
    Unicode warning signs (⚠ △) are not word characters, so they are captured as
    prefix/suffix and never sent to the translation model, preventing corruption
    or loss.
    """
    i = 0
    while i < len(text) and not _is_word_char(text[i]):
        i += 1
    j = len(text)
    while j > i and not _is_word_char(text[j - 1]):
        j -= 1
...
# ── Phonetic Devanagari → English for loanword sounding (reverse of above) ──
# Used when user tags <टर्म> on hi->en: saves the sounding, not a translation.
# e.g. स्प्रिंग→spring, मोटर→motor, लेग→leg, ब्रश→brash
...
_DEVA_VELAR_NASAL_TRIGGERS = set('कखगघङ')
_DEVA_LABIAL_NASAL_TRIGGERS = set('बभपफम')


def _deva_phonetic_to_en(word):
    """Phonetic Devanagari → English (loanword style). e.g. स्प्रिंग→spring, लेग→leg."""
    w = word.strip()
    out = []
    i = 0
    n = len(w)
    ...
    while i < n:
        ...
        if ch in _DEVA_INDEP_VOWELS:
            out.append(_DEVA_INDEP_VOWELS[ch])
            i += 1
            continue

        if ch in _DEVA_CONSONANTS:
            cons = _DEVA_CONSONANTS[ch]
            if nxt == '्':                       # virama: consonant cluster, no vowel
                out.append(cons)
                i += 2
                continue
            if nxt in _DEVA_MATRAS:               # matra replaces inherent 'a'
                out.append(cons + _DEVA_MATRAS[nxt])
                i += 2
                continue
            # Bare consonant: drop the inherent 'a' if this is the last consonant
            # in the word (schwa deletion), otherwise keep it.
            trailing = w[i + 1:]
            if trailing.strip('ंँः') == '':
                out.append(cons)
            else:
                out.append(cons + 'a')
            i += 1
        ...
        if ch == 'ं':                             # anusvara: nasal, quality from next consonant
            if nxt in _DEVA_VELAR_NASAL_TRIGGERS:
                out.append('ng')
            elif nxt in _DEVA_LABIAL_NASAL_TRIGGERS:
                out.append('m')
            else:
                out.append('n')
            i += 1
        ...
        if ch == '्':                             # stray virama
            i += 1
            continue

        out.append(ch)                            # passthrough (spaces, digits, punctuation)
        i += 1
...
def _auto_add_tagged_terms(core, engine):
    """Extract <term> tags from text, add new terms to glossary.

    Returns (cleaned_text, tagged_terms) — tagged_terms maps each term tagged in
    THIS call to its Hindi phonetic sounding, so callers can force-protect it via
    placeholder substitution even for engines (like IT3) that only receive the
    glossary as a soft prompt hint and may otherwise translate it anyway.
    """
    ...
    existing_lower = {k.lower(): k for k in _glossary}
    ...
    for term in dict.fromkeys(matches):          # preserve order, deduplicate
        ...
        if term.lower() in existing_lower:       # already in glossary (any casing)
            tagged_terms[term] = _glossary[existing_lower[term.lower()]]
            continue
    ...
    return _RE_ANGLE_TAG.sub(r'\1', core), tagged_terms


def _auto_add_tagged_terms_hi(core):
    """Extract <term> tags (Hindi) from text, add new terms to the hi->en glossary
    as phonetic English sounding.

    Returns (cleaned_text, tagged_terms) — see _auto_add_tagged_terms for why.
    """
    matches = _RE_ANGLE_TAG.findall(core)
    if not matches:
        return core, {}

    existing = set(_glossary_hi)
    deleted  = set(_overrides.get('deleted_hi', []))
    ...
    for term in dict.fromkeys(matches):           # preserve order, deduplicate
        ...
        if term in deleted:
            continue
        if term in existing:                      # already learnt
            tagged_terms[term] = _glossary_hi[term]
            continue

        # Sound the Devanagari out in English letters — workshop terms must keep
        # their English sounding, not be looked up as a semantic translation.
        english = _deva_phonetic_to_en(term)

        if not english:
            english = term

        _overrides.setdefault('learnt_hi', {})[term] = english
        tagged_terms[term] = english
        changed = True
        print(f"  Auto-glossary (hi->en): <{term}> → {english}")
...
def _fix_english_glossary_labels(text, glossary):
    """Replace English glossary terms at line-starts (e.g. 'Dust Protective Equipment: Hindi…')
    with their Hindi equivalents.  Handles lines that begin with non-word bullet characters
    like □ by preserving them and replacing only the English label portion."""
    if not glossary or not text:
        return text
    for eng, hi in sorted(glossary.items(), key=lambda x: -len(x[0])):
        # ([^\w]*) captures any leading bullet/symbol; \g<1> puts it back
        text = re.sub(
            r'(?im)^([^\w]*)' + re.escape(eng) + r'(?=\s*:)',
            lambda m, h=hi: m.group(1) + h,
            text
        )
    return text
...
def translate_text(text, from_lang="hi", to_lang="en", engine="auto"):
    ...
    tagged_terms = {}
    if from_lang == 'en' and to_lang == 'hi':
        text, tagged_terms = _auto_add_tagged_terms(text, engine)
    elif from_lang == 'hi' and to_lang == 'en':
        text, tagged_terms = _auto_add_tagged_terms_hi(text)
    ...
    restore_map = {}
    # IT3 receives the full glossary as a soft prompt hint (see _translate_with_it3);
    # other engines get it enforced via placeholders.
    if from_lang == 'en' and to_lang == 'hi' and _glossary and engine != 'it3':
        core, restore_map = _apply_glossary(core, _glossary)
    elif from_lang == 'hi' and to_lang == 'en' and _glossary_hi and engine != 'it3':
        core, restore_map = _apply_glossary(core, _glossary_hi)

    # <tag>'d terms must be force-protected regardless of engine — IT3's prompt hint
    # is only a suggestion and it will translate/paraphrase the term otherwise.
    if tagged_terms:
        core, tag_restore_map = _apply_glossary(core, tagged_terms)
        restore_map.update(tag_restore_map)
    ...
    if to_lang == 'hi':
        if engine == 'it3' and _glossary:
            result = _fix_english_glossary_labels(result, _glossary)
        result = _postprocess_hindi(result)

    return prefix + result + _normalize_terminal_suffix(suffix, result, to_lang)
